                    ...
                    {props.galleryImages[0] !== ''?
                        <div >
                            <div style={{borderRadius:'8px' , padding:'0px 5px 0px 5px'}} className={Style.subPhotoDiv}>
                                <img title={props.title} alt={props.title} onClick={(e)=>{setCurrentImage(e.target.src); setSelectedImage(e.target.alt)}} src={props.galleryImages[0] === '' ? `${PlaceHolderImg}`:`${props.galleryImages[0]}`} className={selectedImage === 'تصویر محصول 1' ? `${Style.activeImage}` : `${Style.subImg}`}></img>
                            </div>
                        </div>
                    :null}
